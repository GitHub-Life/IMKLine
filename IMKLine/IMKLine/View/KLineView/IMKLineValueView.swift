...
class IMKLineValueView: UIView {
    ...
    func update(kline: IMKLine) {
        ...
        (self.subviews[0] as! UILabel).text = String.init(format: "\(self.valueTips[0])%.2f", kline.open)
        (self.subviews[1] as! UILabel).text = String.init(format: "\(self.valueTips[1])%.2f", kline.high)
        (self.subviews[2] as! UILabel).text = String.init(format: "\(self.valueTips[2])%.2f", kline.low)
        (self.subviews[3] as! UILabel).text = String.init(format: "\(self.valueTips[3])%.2f", kline.close)
